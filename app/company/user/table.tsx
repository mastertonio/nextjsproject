...
const CompanyUserTable: React.FC<ICompanyUserTableProps> = ({ update }) => {
  ...
  const [sortBy, setSortBy] = useState<keyof ICompanyTemplateUsersProps | null>(null);
  ...
  const companyUsers = currentPosts?.map(
    (item: ICompanyTemplateUsersProps) => ({
      ...
      actions: (
        ...
        >
          <TransferSingleButton id={item.user_id} name={item.username} refetch={update} tempId={item.template_id}/>
        </div>
      ),
    })
  );
