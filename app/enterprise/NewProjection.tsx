...
const NewProjection = ({
    ...
    const [newContent, setNewContent] = useState<NewContentProps[]>([])
    // const newColumns =
    //     newContent.length > 0
    //         ? newContent.slice(0, 3).map((elem, index) => (
    //             <>
    //                 <Grid.Col
    //                     span="auto"
    //                     key={index}
    ...
    //                 >
    //                     <div dangerouslySetInnerHTML={{ __html: elem.sectionWriteUp }}></div>
    //                 </Grid.Col>
    //                 <Suspense fallback={`Loading...`}>
    //                     <Grid.Col key={index} span="auto" className="sm:w-[40%] w-[100%]">
    //                         <VideoMedia media={elem.sectionVideoLink} />
    //                     </Grid.Col>
    //                 </Suspense>
    //             </>
    //         )) : null;
    ...
    useEffect(() => {
        const elementContent = JSON.parse(localStorage.getItem("valueBucket") ?? '[]') as NewContentProps[];
        setNewContent(elementContent)
    }, [])
